  ...
  <div class="contract-receipts add-advertise">
    ...
    <el-form class="c-search-form" :rules="rules" :model="ruleForm" ref="ruleForm" label-width="145px" :inline="true"
      ...
      <el-row>
        ...
        <el-col :span="10">
          <el-form-item label="上架时间" prop="initialTime">
            <el-date-picker
              v-model.trim="ruleForm.initialTime"
              align="right"
              type="date"
              placeholder="选择日期"
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          initialTime: [{required: true, message: '请选择上架时间', trigger: 'change'}],
    ...
    methods: {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
           // let bdSize=this.ruleForm.bdSize;
           // this.ruleForm.bdSize=Number(bdSize).toFixed(2);
           this.$http.post(api.advertiseList,this.ruleForm).then(res=>{
              if(res.code===0){
    ...
    /*watch: {
      '$route' (to, from) {
        if(to.path === '/operation/adsense/addAdsense'){
