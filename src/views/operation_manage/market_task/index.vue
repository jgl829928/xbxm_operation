<template>
  <div class="contract-receipts market-task">
    <el-form  class="c-search-form" :model="pageParams" ref="pageParams" label-width="80px" :inline="true"
              label-position="right">
      <el-row>
        <el-col :span="8">
          <el-form-item label="省" prop="itemName">
            <el-select v-model.trim="pageParams.areaId" clearable placeholder="请选择">
              <el-option
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button class="btn-handle" type="primary" @click="openJudgeData(scope.row.monthTaskQuotaId)"
                       ...
                       :disabled="!(scope.row.auditStatus?(scope.row.auditStatus.code===2):false)||!($store.getters.getPermission('marketTask_2'))"
                       >审核</el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      getOptionData(){
        //省下拉框
        let provinceOption=this.$http.get(api.provinceOption);
        //商场名称下拉框
        let storeNameOption=this.$http.get(api.allStore);
        Promise.all([provinceOption,storeNameOption]).then(res=>{
          if(res[0].code===0){
            this.provinceOption=[];
            this.provinceOption=[...res[0].data];
          }
          if(res[1].code===0){
      ...
      async getPageList(index) {
        ...
        let res=await this.$http.get(api.marketMonthList,{params:this.pageParams});
        if(res.code===0){
