  ...
  <div class="g-add-company">
    <el-form  class="c-search-form" :model="reqData" ref="reqData" :rules="rules" label-width="110px"
       ...
       <select-market :reqData="reqData" :checkMarket="checkEdit"></select-market>
        ...
        <el-col :span="12" :xs="24" class="u-input-260">
      ...
      <el-row :gutter="32">
       <el-col :span="11" :xs="24">
         ...
         <el-col :span="11" class="u-input-260">
...
export default {
  ...
  data() {
    ...
    ratesStandard:[{ validator: ratesStandardNum, trigger: 'blur' },
  ...
  created(){
    this.getEditData(this.$route.params.companyId)
    this.getService()
  },
  computed:{
    gTicket:function(){
  ...
  methods: {
   submitForm(formName) {
     ...
     reset(form){
      ...
      //获取编辑页内容
      getEditData(params){
        this.$http.get(ApiSetting.company,{params:{companyId:params}}).then(res => {
          if(res && res.data){
            if(res.code == 0){
              this.reqData= keysFilter(res.data[0],true,['marketName','serveName','createUser','createName','createTime','contractBeginTime','contractEndTime'])
               if(res.data[0].imgUrl){
                 this.fileList = res.data[0].imgUrl
                }
                this.$set(this.reqData,'time',[])
                if(res.data[0].contractEndTime && res.data[0].contractBeginTime){
                   this.reqData.time.push(res.data[0].contractBeginTime)
                  this.reqData.time.push(res.data[0].contractEndTime)
                }
            }
          }
        })
