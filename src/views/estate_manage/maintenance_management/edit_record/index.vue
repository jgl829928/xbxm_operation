       ...
       <el-row :gutter="32">
         <el-col :span="11" :xs="24">
          <el-form-item label="设备编号" prop="equipmentCode">
              <el-select v-model="reqData.equipmentCode" clearable placeholder="请选择" @change="getMertion" disabled>
                <el-option
...
export default {
 ...
 components:{selectMarket},
   created(){
      this.pickerCheck = this.pickerOptions
      this.getEditData(this.$route.params.maintenanceId)
   ...
   submitForm(formName) {
    ...
    handleCategory(val,list){
      ...
      getEquipmentCodeList(){
        this.$http.get(ApiSetting.allCode).then(res => {
          if(res){
            if(res.code == 0) {
       ...
       //获取编辑页内容
      getEditData(params){
        this.$http.get(ApiSetting.maintenance,{params:{maintenanceId:params}}).then(res => {
          if(res && res.data){
            if(res.code == 0){
              this.reqData= keysFilter(res.data[0],true,['beginTime','endTime','createUser','marketName','createName','createdTime','rankName','rankSecId','rankId'])
                  this.$set(this.reqData,'time',[])
                  let str = res.data[0].endTime.replace(/-/g,'/');
                  let endtime = new Date(str).getTime()
                  let str1 = res.data[0].beginTime.replace(/-/g,'/');
                  let strattime = new Date(str1).getTime()
                  this.reqData.time.push(strattime)
                  this.reqData.time.push(endtime)
                if(res.data[0].imgAccessoryUrl){
                  this.accessoryList = res.data[0].imgAccessoryUrl
                }
                if(res.data[0].imgContractUrl){
                 this.contractList = res.data[0].imgContractUrl
                }
                this.$set(this.reqData,'firstId',res.data[0].rankId)
                this.handleCategory(this.reqData.firstId, "categotyListSec");
                this.$nextTick(() => {
                  this.$set(this.reqData,'rankId',res.data[0].rankSecId || "")
                  this.getMertion(res.data[0].equipmentCode)
                })
            }
          }
        })
