        ...
        <el-col :span="12">
          <el-form-item label="维修类别" prop="typeName">
            <el-select v-model="reqData.typeName" clearable placeholder="请选择">
              ...
              <div>
                  <el-input v-model="typeName" v-if="show_input" placeholder="请输入类别名称"></el-input>
              </div>
...
export default {
  ...
  methods: {
    getRepairInfo() {
      let repairInfo = `${ApiSetting.repairList}/${
        this.$route.params.repairId
      }`;
      this.$http.get(repairInfo).then(res => {
        if (res && res.code == 0) {
          res.data.time = [];
          res.data.time.push(res.data.repairStartDate);
          res.data.time.push(res.data.repairEndDate);
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          delete data.time;
          let editRepair = `${ApiSetting.operate_repair}/${
            this.$route.params.repairId
          }`;
          this.$http.put(editRepair, data).then(res => {
            if (res && res.code == 0) {
              this.$message({
                message: "保存成功",
                type: "success"
