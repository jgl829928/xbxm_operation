<template>
  <div class="contract-receipts add-merchant">
      ...
      <el-row class="brand-wrap">
        ...
        <el-col :span="7">
          <el-form-item label="类别" prop="mainType" label-width="60px">
            <!--<el-select v-model.trim="reqData.merchantBrandList[0].brandClass" clearable placeholder="请选择">-->
...
<script>
  ...
  export default {
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            console.log(this.reqData.merchantBrandList);
            this.$http.post(api.merchants, this.reqData).then(res => {
              if (res.code === 0) {
                _this.$message.success(res.msg);
                this.$store.commit("delete_tabs", this.$route);
                _this.$router.push({path: '/merchant/merchantManage'})
              }
            });
          } else {
   ...
   /* watch: {
      '$route'(to, from) {
        if (to.path === '/merchant/merchantManage/addMerchant') {
          Object.assign(this.reqData, this.$options.data().reqData);
          setTimeout(() => {
            this.$refs['ruleForm'].clearValidate();
          }, 20);
          Object.assign(this.merchantBrandList1, this.$options.data().merchantBrandList1);
          Object.assign(this.merchantBrandList2, this.$options.data().merchantBrandList2);
        }
      }
    }*/
  };
