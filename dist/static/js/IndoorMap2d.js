...
IndoorMap2d = function(mapdiv){
    ...
    this.init = function(){
        // console.log(_mapDiv.style.width)
...
//            _marker.style.visibility = true;
    ...
    function onSelectObject(event){
        ...
        if(event.type == "touchend"){
            ...
            pos[1] = event.changedTouches[0].clientY+$('.el-main').scrollTop();
        }else {
            pos[0] = event.clientX;
            pos[1] = event.clientY+$('.el-main').scrollTop();
        }
        if(Math.abs(pos[0] - _controls.startPoint[0] + pos[1] - (_controls.startPoint[1]+$('.el-main').scrollTop())) <5) {
            ...
            if (_selected) {
                ...
                if(_this.getSelectedId()){
                  $.ajax({
                     type: "get",
                     url:httpApi,
                     data :{boothCode:_this.getSelectedId()},
                     beforeSend: function (XMLHttpRequest) {
                       XMLHttpRequest.setRequestHeader("gTicket", window.sessionStorage.getItem('optimize_gTicket'));
                     },
                     success: function (res) {
                       if(res.code==0){
                         $('.dialog-map').removeClass('displayFla');
                         $('.masking').removeClass('displayFla');
                         var boothStatusText
                         if(res.data.boothStatus==1){
                           boothStatusText='未出租'
                         }else if(res.data.boothStatus==2){
                           boothStatusText='签约待审核'
                         }else if(res.data.boothStatus==3){
                           boothStatusText='已出租'
                         }else if(res.data.boothStatus==4){
                           boothStatusText='交款期'
                         }else if(res.data.boothStatus==5){
                           boothStatusText='欠款待缴费'
                         }else if(res.data.boothStatus==6){
                           boothStatusText='合同即将到期'
                         }else if(res.data.boothStatus==7){
                           boothStatusText='已预定'
                         }


                         $('.dialog-map').find('.boothNumber').text(res.data.boothNumber || '')
                         $('.dialog-map').find('.boothCode').text(res.data.boothCode || '')
                         $('.dialog-map').find('.marketName').text(res.data.marketName || '')
                         $('.dialog-map').find('.boothType').text(res.data.boothType || '')
                         $('.dialog-map').find('.manager').text(res.data.manager || '')
                         $('.dialog-map').find('.floor').text(res.data.floor+'楼')
                         $('.dialog-map').find('.area').text(res.data.area || '')
                         $('.dialog-map').find('.boothStatus').text(boothStatusText || '')
                         $('.dialog-map').find('.calculateArea').text(res.data.calculateArea.toFixed(2) || '')
                         $('.dialog-map').find('.usableArea').text(res.data.usableArea.toFixed(2) || '')
                         $('.dialog-map').find('.boothPrice').text(res.data.boothPrice.toFixed(2) || '')
                         $('.dialog-map').find('.units').text(res.data.units.desc || '')
                         $('.dialog-map').find('.contractBatchNum').text(res.data.contractBatchNum || '')
                         $('.dialog-map').find('.merchantName').text(res.data.merchantName || '')
                         $('.dialog-map').find('.rentBeginTime').text(res.data.rentBeginTime || '').attr({'title':res.data.rentBeginTime})
                         $('.dialog-map').find('.rentEndTime').text(res.data.rentEndTime || '').attr({'title':res.data.rentEndTime})
                       }
                  ...
                  $.get(httpApi,{boothCode:_this.getSelectedId()},function(res){


                  })
                  // vueThis.$http.get(httpApi,{params: {boothCode:_this.getSelectedId()}}).then(res=>{
                  //
                  // })
...
Canvas2DRenderer = function (map) {
    ...
    var _padding = 0.1;  //padding between map bounding box and the div boundary
    ...
    function clipPadding(){
        ...
        return Math.max(0, Math.min(0, ratio));
    }
    ...
    this.render = function (){
        ...
        if(options.showNames){
            ...
            for(var i = 0 ; i < funcAreas.length; i++){
                ...
                if(nameText.visible) {
                  ...
                  _ctx.fillText(nameText.text.substring(0,nameText.text.length-3) + "..", (center[0] - nameText.halfWidth+12) >> 0, (-center[1]) >> 0);
                }
            }
        }
        ...
        if(options.showPubPoints){
            ...
            for(var i = 0; i < pubPoints.length; i++){
                ...
                imgWidth = pubPoint.width;
                imgHeight = pubPoint.height;
    ...
    function hitTest(point){
        ...
        for(var i = 0 ; i < _curFloor.FuncAreas.length; i++) {
            ...
            if((!funcArea.Category) && parseInt(funcArea.Type) == 100 || (!funcArea.Category) && parseInt(funcArea.Type) == 1000){ //hollow area
                continue;
            }
    ...
    this.createNameTexts = function(floorId, mall){
        ...
        for(var i = 0 ; i < funcAreaJson.length; i++){
            ...
            if((!funcArea.Category) && ((parseInt(funcArea.Type) == 100) || (parseInt(funcArea.Type) == 300 || (!funcArea.Category) && parseInt(funcArea.Type) == 1000))){
